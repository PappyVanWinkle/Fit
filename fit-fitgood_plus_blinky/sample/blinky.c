//*****************************************************************************
//
// blinky.c - Simple example to blink the on-board LED.
//
// Copyright (c) 2013-2015 Texas Instruments Incorporated.  All rights reserved.
// Software License Agreement
// 
// Texas Instruments (TI) is supplying this software for use solely and
// exclusively on TI's microcontroller products. The software is owned by
// TI and/or its suppliers, and is protected under applicable copyright
// laws. You may not combine this software with "viral" open-source
// software in order to form a larger program.
// 
// THIS SOFTWARE IS PROVIDED "AS IS" AND WITH ALL FAULTS.
// NO WARRANTIES, WHETHER EXPRESS, IMPLIED OR STATUTORY, INCLUDING, BUT
// NOT LIMITED TO, IMPLIED WARRANTIES OF MERCHANTABILITY AND FITNESS FOR
// A PARTICULAR PURPOSE APPLY TO THIS SOFTWARE. TI SHALL NOT, UNDER ANY
// CIRCUMSTANCES, BE LIABLE FOR SPECIAL, INCIDENTAL, OR CONSEQUENTIAL
// DAMAGES, FOR ANY REASON WHATSOEVER.
// 
// This is part of revision 2.1.2.111 of the EK-TM4C1294XL Firmware Package.
//
//*****************************************************************************

#include <stdint.h>
#include <stdbool.h>
#include "inc/hw_memmap.h"
#include "driverlib/gpio.h"
#include "driverlib/sysctl.h"
#include "fit_api.h"
#include "internal.h"
#include "hwdep.h"
#include "mem_read.h"
#include "fit_debug.h"
#include "mbedtls/config.h"
#include "mbedtls/platform.h"
#include "mbedtls/pk.h"
#include "fit_pubkey.h"
/*
 const unsigned char license[] = { // Perpetual, Feature 1
                    };
*/

 const unsigned char license[] = { // Perpetual, Feature 10 (Blinky)
		 	 	    };

 /****************************************************************************/

 void setup(void)
 {
     fit_board_setup();

     /*
      * Set current time to current unixtime. You can customize that according to
      * your hardware capabilities
      */
     FIT_TIME_SET(1453730468); /* Jan 25, 2016 */

     fit_led_off();
 }

 /****************************************************************************/


//*****************************************************************************
//
//! \addtogroup example_list
//! <h1>Blinky (blinky)</h1>
//!
//! A very simple example that blinks the on-board LED using direct register
//! access.
//
//*****************************************************************************

//*****************************************************************************
//
// Blink the on-board LED.
//
//*****************************************************************************
int
main(void)
{

    fit_status_t     status   = FIT_STATUS_OK;
    fitcontextdata context  = {0};
    fit_pointer_t fitptrlic = {0};
    fit_pointer_t fitptrkey = {0};
    const uint16_t no_blink = 0, f_blink = 10; //Blink feature = 10
	volatile uint32_t ui32Loop;

    setup(); /* initialize UART and LED ports */

//    DBG(FIT_TRACE_INFO, "\nTest case: Consume feature in License ---------\n");

    /*
     * call consume license for license in the RAM
     */
    fitptrlic.data = (uint8_t *) license;
    fitptrlic.length = sizeof(license);
    fitptrlic.read_byte = (fit_read_byte_callback_t) READ_BYTE_RAM;

    fitptrkey.data = (uint8_t *) pubkey;
    fitptrkey.length = sizeof(pubkey);
    fitptrkey.read_byte = (fit_read_byte_callback_t) READ_BYTE_RAM;

    // Change 2nd parameter to "f_blink" in order to blink LED ("license" == 10, or
    // "no_blink" to not blink LED
    status = fit_licenf_consume_license(&fitptrlic, f_blink, &context, &fitptrkey);

    fit_trace_flags = FIT_TRACE_ALL;
    if(status==FIT_STATUS_OK)
    {

    	DBG(FIT_TRACE_INFO, "Blink feature found.\n");
    //
    // Enable the GPIO port that is used for the on-board LED.
    //
		SysCtlPeripheralEnable(SYSCTL_PERIPH_GPION);

		//
		// Check if the peripheral access is enabled.
		//
		while(!SysCtlPeripheralReady(SYSCTL_PERIPH_GPION))
		{
		}

		//
		// Enable the GPIO pin for the LED (PN0).  Set the direction as output, and
		// enable the GPIO pin for digital function.
		//
		GPIOPinTypeGPIOOutput(GPIO_PORTN_BASE, GPIO_PIN_0);

		//
		// Loop forever.
		//
		while(1)
		{
			//
			// Turn on the LED.
			//
			GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_0, GPIO_PIN_0);

			//
			// Delay for a bit.
			//
			for(ui32Loop = 0; ui32Loop < 2000000; ui32Loop++)
			{
			}

			//
			// Turn off the LED.
			//
			GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_0, 0x0);

			//
			// Delay for a bit.
			//
			for(ui32Loop = 0; ui32Loop < 2000000; ui32Loop++)
			{
			}
		}

    }
    else
    	    	DBG(FIT_TRACE_INFO, "No valid blink feature found.\n");
}
